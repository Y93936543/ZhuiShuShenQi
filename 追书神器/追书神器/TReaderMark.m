...
{
    NSString *contion = [NSString stringWithFormat:@"bookId = '%@'",bookId];
    return [self dbObjectsWhere:contion orderby:nil];
}

+ (NSArray *)dbObjectsWithBookId:(NSString*)bookId chapterName:(NSString *)chapterName
{
    NSString *contion = [NSString stringWithFormat:@"bookId = '%@' and chapterIndex = '%@'",bookId,chapterName];
    return [self dbObjectsWhere:contion orderby:nil];
}
...
- (BOOL)removeDbObjectsWhereRange:(NSRange)range
{
    ...
    for (TReaderMark *mark in array) {
        if (NSLocationInRange(mark.offset, range)) {
            NSString *contion = [NSString stringWithFormat:@"bookId = '%@' and chapterIndex = '%@' and offset = '%ld'",_bookId,_chapterIndex,mark.offset];
